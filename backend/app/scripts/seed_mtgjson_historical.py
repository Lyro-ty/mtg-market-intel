"""
Seed marketplaces with MTGJSON 90-day historical price data.

This script:
1. Fetches all cards from the database
2. For each card, fetches 90 days of historical price data from MTGJSON
...
async def seed_mtgjson_historical(
    ...
    """
    Seed marketplaces with MTGJSON 90-day historical data.
            ...
            for batch_start in range(0, len(all_cards), batch_size):
                ...
                for card in batch:
                    try:
                        ...
                        # Collect snapshot records for bulk insert
                        snapshot_records = []
                        ...
                        # Pre-fetch/create marketplaces we need
                        marketplaces = {}
                        for currency, (slug, name, base_url) in marketplace_map.items():
                            marketplaces[currency] = await get_or_create_marketplace(
                                db, slug, name, base_url, currency
                            )
                        ...
                        for price_data in historical_prices:
                            ...
                            marketplace = marketplaces.get(price_data.currency)
                            if not marketplace:
                                continue

                            # Add normal (non-foil) price record
                            ...
                            # Add foil price as separate record if available
...
async def main():
    parser = argparse.ArgumentParser(
        description="Seed marketplaces with MTGJSON 90-day historical data"
    )
    parser.add_argument(
        "--card-limit",
        type=int,
    ...
    print(f"\n{'='*60}")
    print("  Seed MTGJSON Historical Data (90 days)")
    print(f"{'='*60}\n")
    ...
    print(f"  Cards skipped (no data): {stats['cards_skipped']:,}")
