...
async def count_mock_data(session, mock_mp: Marketplace | None) -> dict:
    ...
    if mock_mp:
        # Count listings from mock marketplace
        result = await session.execute(
            select(Listing).where(Listing.marketplace_id == mock_mp.id)
        )
        counts["mock_listings"] = len(result.scalars().all())
    ...
    # Count listings from non-real marketplaces
    if real_mp_ids:
        result = await session.execute(
            select(Listing).where(~Listing.marketplace_id.in_(real_mp_ids))
        )
        counts["non_real_listings"] = len(result.scalars().all())
...
async def remove_non_real_listings(session, real_mp_ids: set[int]) -> int:
    """Remove listings from non-real marketplaces."""
    if not real_mp_ids:
        # If no real marketplaces exist, don't delete anything
        return 0
    
    result = await session.execute(
        delete(Listing).where(~Listing.marketplace_id.in_(real_mp_ids))
    )
    await session.flush()
...
async def identify_synthetic_snapshots(session) -> list[int]:
    """
    ...
    for (card_id, marketplace_id), snapshots in snapshots_by_card_marketplace.items():
        ...
        if len(snapshots) == 31:  # 30 days + today = 31 snapshots
            ...
            for i in range(1, len(snapshots)):
                delta = snapshots[i].snapshot_time - snapshots[i - 1].snapshot_time
            ...
            if all(0.9 <= interval <= 1.1 for interval in intervals):
                ...
                if num_listings_values:
                    ...
                    if 5 <= min_listings <= 10 and 50 <= max_listings <= 250:
                        synthetic_ids.extend([s.id for s in snapshots])
    
    return synthetic_ids


async def remove_synthetic_snapshots(session, synthetic_ids: list[int]) -> int:
    """Remove identified synthetic price snapshots."""
    ...
    for i in range(0, len(synthetic_ids), batch_size):
        batch = synthetic_ids[i : i + batch_size]
        result = await session.execute(
...
async def main():
    ...
    async with async_session_maker() as session:
        ...
        print(f"    Listings from mock marketplace: {counts['mock_listings']:,}")
        print(
            f"    Price snapshots from mock marketplace: {counts['mock_price_snapshots']:,}"
        )
        print(f"    Listings from non-real marketplaces: {counts['non_real_listings']:,}")
        print(
            f"    Price snapshots from non-real marketplaces: {counts['non_real_price_snapshots']:,}"
        )
        
        total_to_delete = (
            counts["mock_listings"]
            + counts["mock_price_snapshots"]
            + counts["non_real_listings"]
        ...
        # Delete mock marketplace (cascades to listings and price_snapshots)
        if mock_mp:
            deleted_counts["marketplace"] = await remove_mock_marketplace(session, mock_mp)
            print(f"    ✓ Deleted mock marketplace")
        
        # Delete listings from non-real marketplaces
        deleted_counts["listings"] = await remove_non_real_listings(session, real_mp_ids)
        if deleted_counts["listings"] > 0:
            print(f"    ✓ Deleted {deleted_counts['listings']:,} listings from non-real marketplaces")
        ...
        print(f"  Listings deleted: {deleted_counts['listings']:,}")
